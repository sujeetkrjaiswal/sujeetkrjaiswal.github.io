	...
	redraw(){
		...
		*/
		...
		this.particles.forEach((particle,rowIndex,arr)=>{
			...
			for(let colIndex=0;colIndex<rowIndex;colIndex++){
				...
				*/
				...
				if(dist < this.config.MAX_DIST_SQ){
					//Its within give specified range
					this.context.moveTo(particle.x,particle.y);
					let dirx = particle.x > arr[colIndex].x ? particle.x : arr[colIndex].x;
					let diry = particle.y < arr[colIndex].y ? particle.x : arr[colIndex].y;
					this.context.quadraticCurveTo(dirx,diry,particle.x,particle.y);
			...
			//Related to move hove - particle enlagement
			var disFact = this.disBtwSq(mouse,particle)
		})
